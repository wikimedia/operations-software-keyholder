...
class SshAgentServer(socketserver.ThreadingUnixStreamServer):
    ...
    # Mark threads as daemon in order to prevent Python 3.7+ classes that inherit
    # from ThreadingMixIn to keep track of each created thread to be able to wait
    # for them on exit, hence creating a memory leak during the daemon lifetime.
    daemon_threads = True
